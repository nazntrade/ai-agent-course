...
# Titles used by earlier versions for an unnamed chat. A database migrated from
# an older day still carries them, so they must stay eligible for the automatic
...
class DuplicateMemoryKeyError(ValueError):
    """Raised when a memory key already exists in the target scope.

    ``key`` keeps the cleaned key the caller tried to insert; nothing is
    written, so the existing entry stays untouched.
    """
...
-- Working memory is bound to a chat line: ``branch_id`` NULL is the main line,
-- a branch id otherwise. Long-term memory has no scope and deliberately has no
-- foreign key, so it survives deleting the chat or branch it was promoted from.
...
CREATE INDEX IF NOT EXISTS idx_working_memory_scope
    ON working_memory(chat_id, branch_id, id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_working_memory_main_key
    ON working_memory(chat_id, item_key) WHERE branch_id IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_working_memory_branch_key
    ON working_memory(chat_id, branch_id, item_key) WHERE branch_id IS NOT NULL;
...
class ChatStore:
    ...
    def load_config(self, chat_id: int) -> dict:
        with closing(self._connect()) as conn:
            row = conn.execute(
                ...
                "sliding_window_messages, facts_window_messages, invariants "
                "FROM chats WHERE id = ?",
                (chat_id,),
            ).fetchone()
        if row is None:
    ...
    def save_config(self, chat_id: int, config) -> None:
        ...
        with closing(self._connect()) as conn:
            conn.execute(
                ...
                "invariants = ?, updated_at = datetime('now') WHERE id = ?",
                (
                    config.system_prompt,
                    config.model,
                    config.temperature,
    ...
    def delete_branch(self, chat_id: int, branch_id: int) -> None:
        ...
        with closing(self._connect()) as conn:
            try:
                ...
                conn.execute(
                    "DELETE FROM working_memory WHERE chat_id = ? AND branch_id = ?",
                    (chat_id, branch_id),
                )
    ...
    @staticmethod
    def _validate_memory_scope(scope, chat_id, branch_id) -> None:
        """Reject unknown scopes and inconsistent scope arguments.

        Long-term memory is global, so passing a chat or branch is a caller
        bug; working memory always requires a chat and may target the main line
        (``branch_id`` None) or one branch.
        """
        if scope not in (MEMORY_SCOPE_WORKING, MEMORY_SCOPE_LONG_TERM):
            raise ValueError(f"Unknown memory scope: {scope}")
        if scope == MEMORY_SCOPE_LONG_TERM:
            if chat_id is not None or branch_id is not None:
                raise ValueError(
                    "Long-term memory is not scoped to a chat or branch"
                )
            return
        if chat_id is None:
            raise ValueError("Working memory requires a chat id")

    def _ensure_memory_scope(self, conn, scope, chat_id, branch_id) -> None:
        """Validate the scope and that the chat/branch exists in the database."""
        self._validate_memory_scope(scope, chat_id, branch_id)
        if scope == MEMORY_SCOPE_LONG_TERM:
            return
        chat = conn.execute(
    ...
    @staticmethod
    def _memory_row_exists(conn, scope, item_id, chat_id, branch_id):
        """Return a truthy row when the scoped item exists."""
        if scope == MEMORY_SCOPE_LONG_TERM:
            return conn.execute(
                "SELECT 1 FROM long_term_memory WHERE id = ?", (item_id,)
            ).fetchone()
        return conn.execute(
            "SELECT 1 FROM working_memory WHERE id = ? AND chat_id = ? "
            "AND branch_id IS ?",
            (item_id, chat_id, branch_id),
        ).fetchone()

    def list_memory_items(
        self, scope, chat_id=None, branch_id=None
    ) -> list[MemoryItem]:
        """Return the scope's entries ordered by id.

        Working memory uses an exact ``branch_id IS ?`` match: a branch never
        inherits entries from its parent, so sibling lines stay isolated.
        """
        with closing(self._connect()) as conn:
            self._ensure_memory_scope(conn, scope, chat_id, branch_id)
            if scope == MEMORY_SCOPE_LONG_TERM:
                rows = conn.execute(
                    "SELECT id, item_key, value, included, updated_at "
                    "FROM long_term_memory ORDER BY id"
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT id, item_key, value, included, updated_at "
                    "FROM working_memory WHERE chat_id = ? AND branch_id IS ? "
                    "ORDER BY id",
                    (chat_id, branch_id),
                ).fetchall()
        return [
    ...
    def add_memory_item(
        self, scope, key, value, chat_id=None, branch_id=None
    ) -> int:
        """Insert one entry and return its id; duplicate keys are rejected."""
        cleaned_key = validate_memory_key(key)
        ...
        with closing(self._connect()) as conn:
            ...
            try:
                if scope == MEMORY_SCOPE_LONG_TERM:
                    cursor = conn.execute(
                        "INSERT INTO long_term_memory (item_key, value) "
                        "VALUES (?, ?)",
                        (cleaned_key, cleaned_value),
                    )
                else:
                    cursor = conn.execute(
                        "INSERT INTO working_memory (chat_id, branch_id, item_key, "
                        "value) VALUES (?, ?, ?, ?)",
                        (chat_id, branch_id, cleaned_key, cleaned_value),
                    )
                conn.commit()
            except sqlite3.IntegrityError as exc:
                conn.rollback()
                raise DuplicateMemoryKeyError(cleaned_key) from exc
            return cursor.lastrowid

    def edit_memory_item(
        self, scope, item_id, key, value, chat_id=None, branch_id=None
    ) -> None:
        """Update key and value of a scoped entry without touching ``included``."""
        cleaned_key = validate_memory_key(key)
        cleaned_value = validate_memory_value(value)
        with closing(self._connect()) as conn:
            self._ensure_memory_scope(conn, scope, chat_id, branch_id)
            if self._memory_row_exists(
                conn, scope, item_id, chat_id, branch_id
            ) is None:
                raise KeyError(f"memory item {item_id} not found")
            try:
                if scope == MEMORY_SCOPE_LONG_TERM:
                    conn.execute(
                        "UPDATE long_term_memory SET item_key = ?, value = ?, "
                        "updated_at = datetime('now') WHERE id = ?",
                        (cleaned_key, cleaned_value, item_id),
                    )
                else:
                    conn.execute(
                        "UPDATE working_memory SET item_key = ?, value = ?, "
                        "updated_at = datetime('now') WHERE id = ? AND chat_id = ? "
                        "AND branch_id IS ?",
                        (cleaned_key, cleaned_value, item_id, chat_id, branch_id),
                    )
                conn.commit()
            except sqlite3.IntegrityError as exc:
                conn.rollback()
    ...
    def forget_memory_item(
        self, scope, item_id, chat_id=None, branch_id=None
    ) -> None:
        """Delete a scoped entry permanently; a missing row raises ``KeyError``."""
        with closing(self._connect()) as conn:
            self._ensure_memory_scope(conn, scope, chat_id, branch_id)
            if scope == MEMORY_SCOPE_LONG_TERM:
                cursor = conn.execute(
                    "DELETE FROM long_term_memory WHERE id = ?", (item_id,)
                )
            else:
                cursor = conn.execute(
                    "DELETE FROM working_memory WHERE id = ? AND chat_id = ? "
                    "AND branch_id IS ?",
                    (item_id, chat_id, branch_id),
                )
            conn.commit()
    ...
    def set_memory_item_included(
        self, scope, item_id, included, chat_id=None, branch_id=None
    ) -> None:
        """Toggle whether a scoped entry is sent to the model."""
        flag = int(bool(included))
        with closing(self._connect()) as conn:
            self._ensure_memory_scope(conn, scope, chat_id, branch_id)
            if scope == MEMORY_SCOPE_LONG_TERM:
                cursor = conn.execute(
                    "UPDATE long_term_memory SET included = ?, "
                    "updated_at = datetime('now') WHERE id = ?",
                    (flag, item_id),
                )
            else:
                cursor = conn.execute(
                    "UPDATE working_memory SET included = ?, "
                    "updated_at = datetime('now') WHERE id = ? AND chat_id = ? "
                    "AND branch_id IS ?",
                    (flag, item_id, chat_id, branch_id),
                )
            conn.commit()
            if cursor.rowcount == 0:
                raise KeyError(f"memory item {item_id} not found")

    def promote_memory_item(self, chat_id: int, branch_id, item_id) -> int:
        """Move a working entry into global long-term memory.

        The move is atomic: the long-term copy is inserted first, so a duplicate
        key leaves the working entry in place and raises
        ``DuplicateMemoryKeyError``. On success the working entry is removed and
        the new long-term id is returned; any failure rolls the whole
        transaction back.
        """
        with closing(self._connect()) as conn:
            try:
                self._ensure_memory_scope(
                    conn, MEMORY_SCOPE_WORKING, chat_id, branch_id
                )
                row = conn.execute(
                    "SELECT item_key, value, included FROM working_memory "
                    "WHERE id = ? AND chat_id = ? AND branch_id IS ?",
                    (item_id, chat_id, branch_id),
                ).fetchone()
                if row is None:
                    raise KeyError(f"memory item {item_id} not found")
                key, value, included = row
                try:
                    cursor = conn.execute(
                        "INSERT INTO long_term_memory (item_key, value, included) "
                        "VALUES (?, ?, ?)",
                        (key, value, included),
                    )
                except sqlite3.IntegrityError as exc:
                    raise DuplicateMemoryKeyError(key) from exc
                conn.execute(
                    "DELETE FROM working_memory WHERE id = ? AND chat_id = ? "
                    "AND branch_id IS ?",
                    (item_id, chat_id, branch_id),
                ...
                for operation in operations:
                    ...
                    if active is None:
                        ...
                        conn.execute(
                            ...
                            "updated_at = datetime('now') WHERE id = ?",
                            ("replaced by a new value", active[0]),
                        )
                        conn.execute(
                            "INSERT INTO facts (chat_id, category, fact_key, value, status) "
                            "VALUES (?, ?, ?, ?, 'active')",
        ...
        with closing(self._connect()) as conn:
            try:
                ...
                if row is not None and (
                    row[0] == DEFAULT_CHAT_TITLE or row[0] in LEGACY_CHAT_TITLES
                ):
