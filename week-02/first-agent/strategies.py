...
and their Russian labels, so storage, the agent and the UI never invent them.
...
# Machine value first, human-readable Russian label second. The order is the
# order shown in the UI selector.
STRATEGY_CHOICES = (
    (STRATEGY_FULL, "Полная история"),
    (STRATEGY_SUMMARY, "Сводка (сжатие)"),
    (STRATEGY_SLIDING, "Скользящее окно"),
    (STRATEGY_FACTS, "Sticky Facts (память фактов)"),
    (STRATEGY_BRANCHING, "Ветвление"),
)
...
def strategy_label(value) -> str:
    """Return the Russian label for a strategy, normalizing unknown values."""
