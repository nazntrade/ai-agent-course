...
The MCP server and the model are replaced by the shared doubles and the chat
database is a temporary file, so these tests need no port and no network. The
HTTP contract, the SSE framing and the controlled error behaviour are still the
...
class _AppTestCase(unittest.TestCase):
    """A backend bound to a temporary database, with a chat already created."""

    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self._tmp.cleanup)
        self.settings = resolve_settings(
            env={"AGENT_DB_PATH": str(Path(self._tmp.name) / "test.sqlite3")},
            dotenv=False,
        )
        self.chats = ChatService(Database(self.settings.db_path))
        self.default_chat_id = self.chats.create_chat("Test chat")["id"]

    def _build_app(self, *, provider=None, mcp_client=None):
        return create_app(
            self.settings,
...
class McpStatusTest(_AppTestCase):
    ...
    def test_status_connected(self):
        ...
        self.assertEqual(payload["tools_count"], len(sample_tools()))
    ...
    def test_tools_match_the_mcp_server(self):
        ...
        app = self._build_app(mcp_client=FakeMcpClient(tools=client_tools))
        with TestClient(app) as client:
    ...
    def test_tools_endpoint_opens_exactly_one_probe(self):
        mcp = FakeMcpClient()
        app = self._build_app(mcp_client=mcp)
        with TestClient(app) as client:
            response = client.get("/api/mcp/tools")
        self.assertEqual(response.status_code, 200)
...
class StaticAssetsTest(_AppTestCase):
...
class ChatStreamTest(_AppTestCase):
    ...
    def test_full_tool_chain(self):
        ...
        mcp = FakeMcpClient()
        app = self._build_app(provider=provider, mcp_client=mcp)
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream",
                json={"chat_id": self.default_chat_id, "message": "What is 23 * 17?"},
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_status_events_carry_the_request_id_of_done(self):
        provider = ScriptedProvider([_tool_turn(), _answer_turn("The result is 391.")])
        app = self._build_app(provider=provider)
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream",
                json={"chat_id": self.default_chat_id, "message": "What is 23 * 17?"},
    ...
    def test_second_request_in_the_same_chat_reuses_history(self):
        provider = ScriptedProvider([_answer_turn("First."), _answer_turn("Second.")])
        app = self._build_app(provider=provider)
        with TestClient(app) as client:
            client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": "one"}
            )
            client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": "two"}
            )
    ...
    def test_messages_of_other_chats_are_not_sent(self):
        other_id = self.chats.create_chat("Other")["id"]
        self.chats.append_exchange(other_id, "secret one", "secret two")
        provider = ScriptedProvider([_answer_turn("Answer.")])
        app = self._build_app(provider=provider)
        with TestClient(app) as client:
            client.post(
                "/api/chat/stream",
                json={"chat_id": self.default_chat_id, "message": "hello"},
            )
        rendered = json.dumps(provider.requests[0]["messages"])
    ...
    def test_empty_message_is_rejected(self):
        with TestClient(self._build_app()) as client:
            response = client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": ""}
            )
    ...
    def test_unknown_chat_returns_a_404_chat_not_found(self):
        with TestClient(self._build_app()) as client:
            response = client.post(
                "/api/chat/stream",
                json={"chat_id": "missing-chat", "message": "hi"},
            )
    ...
    def test_mcp_unavailable_error_event(self):
        ...
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": "hi"}
            )
        events = _parse_sse(response.text)
        names = [name for name, _ in events]
    ...
    def test_model_unavailable_error_event(self):
        ...
        app = self._build_app(provider=FailingProvider())
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": "hi"}
            )
    ...
    def test_model_not_configured_error_event_before_mcp(self):
        ...
        mcp = FakeMcpClient()
        app = self._build_app(provider=UnconfiguredProvider(), mcp_client=mcp)
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream", json={"chat_id": self.default_chat_id, "message": "hi"}
            )
    ...
    def test_unicode_message_round_trip(self):
        ...
        with TestClient(app) as client:
            response = client.post(
                "/api/chat/stream",
                json={"chat_id": self.default_chat_id, "message": "Привет, посчитай 2 + 2"},
            )
        events = _parse_sse(response.text)
        ...
        self.assertIn(
            "Привет, посчитай 2 + 2",
            json.dumps(provider.requests[0]["messages"], ensure_ascii=False),
        )
